...
export default function CorpusZone({ corpusFiles, setCorpusFiles }: CorpusZoneProps) {
  ...
  const removeCorpus = (uriToRemove: string) => {
    setCorpusFiles((prev) => prev.filter((file) => file.uri !== uriToRemove));
  };

  const handleClearCloud = async () => {
    if (!confirm("¿Estás seguro de que deseas eliminar TODOS los archivos del proyecto en Google AI Studio? Esto borrará el corpus y los borradores alojados.")) return;
