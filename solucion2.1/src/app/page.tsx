...
export default function Home() {
  ...
  useEffect(() => {
    async function init() {
      // Cargamos el corpus independientemente del estado de Gemini
      const filesRes = await listVectorStoreDocuments();
      if (filesRes.success && filesRes.files) {
        setCorpusFiles(filesRes.files);
      }
      // Verificamos Gemini en paralelo (no bloquea la carga del corpus)
      ...
      <header className="w-full bg-white dark:bg-zinc-900 border-b border-zinc-200 dark:border-zinc-800 sticky top-0 z-10 px-6 py-4">
        <div className="max-w-5xl mx-auto flex items-center space-x-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs font-semibold text-zinc-500 tracking-wider uppercase">ChromaDB RAG · gemini-2.5-flash-lite</p>
          </div>
